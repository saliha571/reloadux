...
import type { UIUXDesignPageData } from "@/lib/types";
...
import pageStyles from "./UIUXDesignPage.module.css";

interface Props {
  data: UIUXDesignPageData;
}

export function UIUXDesignPageClient({ data }: Props) {
  ...
  return (
    <>
      ...
      <CTASection
        ...
        hideLogos
        backgroundImage="/images/covers/web-design-go-live-section-bg.webp"
        accentWord="make it happen"
      ...
      <MVPProcessSection
        ...
        headingClassName={pageStyles.processHeadingBold}
      />

      <section className={pageStyles.servicesSection}>
        <div className={pageStyles.servicesInner}>
          <div className={pageStyles.servicesHeader}>
            <SectionTag text={data.uiuxServices.tag} />
            <h2 className={pageStyles.servicesHeading}>{data.uiuxServices.heading}</h2>
          </div>

          <div className={pageStyles.servicesGrid}>
            {data.uiuxServices.cards.map((card) => (
              <Link key={card.title} href={card.href} className={pageStyles.serviceCard}>
                <div className={pageStyles.serviceCardText}>
                  <p className={pageStyles.serviceCardTag}>[ {card.tag} ]</p>
                  <p className={pageStyles.serviceCardTitle}>{card.title}</p>
                </div>
                <span className={pageStyles.serviceCardArrow} aria-hidden>
      ...
      <CTASection
        ...
        buttonAfterSubtitle
        hideLogos
        backgroundImage="/images/covers/ui-ux-cta-bg.png"
        headingClassName={pageStyles.ctaHeading}
        className={pageStyles.midCtaWithBottomBorder}
      />
      ...
      <ServiceFAQSection {...data.faqs} hideHeading />
