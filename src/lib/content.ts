...
export async function getLegacyModernizationPage(): Promise<LegacyModernizationPageData> {
  const fallback = legacyModernizationData as LegacyModernizationPageData;
...
export async function getDesignSystemsPage(): Promise<DesignSystemsPageData> {
  const fallback = designSystemsData as DesignSystemsPageData;

  try {
    const res = await getStrapiDesignSystemsPage();
    const s = res?.data;
    if (!s) return fallback;
